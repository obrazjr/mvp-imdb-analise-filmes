PUC-RJ
Curso: Ciência de Dados e Analytics
Disciplina: Sprint: Engenharia de Dados
Aluno: Otávio Braz Ribeiro Junior

MVP Análise de Filmes (IMDb)

Este repositório apresenta um MVP acadêmico que utiliza dados públicos do IMDb para analisar a avaliação e a popularidade de filmes. O trabalho foi desenvolvido na plataforma Databricks, utilizando consultas SQL para exploração e análise dos dados.

A base de dados contém informações sobre os filmes, como gênero, ano de lançamento e duração, além de dados de avaliação do público, como nota média e número de votos. As análises realizadas buscaram identificar possíveis relações entre essas características e a avaliação atribuída pelos usuários.

Os resultados indicam que filmes mais populares tendem a apresentar notas médias mais altas, enquanto fatores como duração e ano de lançamento não demonstraram influência significativa.

Notebook: notebook/MVP_IMDb_Databricks.ipynb
Relatório em PDF: docs/Trabalho_Final.pdf
Dicionário de Dados: docs/Dicionario_de_Dados_IMDb.pdf
Evidências: screenshots/
README: README.md

